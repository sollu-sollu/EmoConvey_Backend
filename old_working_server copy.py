...
NGROK_AUTH_TOKEN = "3DZ5nthwacSPRgixxV3T9SPXERN_jAoBCxVCS3LJ1de93GJt"
...
# Prompts
# ============================================================
# Rule: Use question-style prompts. The model responds to questions
# better than imperatives like 'STRICT/MANDATORY'. Keep them SHORT.

# Live analysis: audio + face (multimodal turn)
EMOTION_PROMPT = (
    "What did the person say and how do they feel emotionally? "
    "Reply in this EXACT format only (no other text):\n"
    "[TRANSCRIPTION: exact words heard]\n"
    "[Emotion]\n"
    "Brief empathetic reply (max 12 words)."
)

# Background mood check: face image only (silent turn)
IMAGE_ONLY_PROMPT = (
    "What emotion does this person's facial expression show? "
    "Reply in this EXACT format only:\n"
    "[Emotion]\n"
    "One warm sentence about their mood."
)

# Chat text-only turn
# LLaMA-2-chat responds best to natural, conversational questions
# not rigid "EXACT FORMAT ONLY" commands which produce robotic outputs
TEXT_ONLY_PROMPT_TEMPLATE = (
    'Someone said: "{user_text}". '
    "What emotion are they feeling, and what is a warm, genuine, empathetic reply? "
    "Start your answer with [Emotion] on its own line, then write your natural reply."
)
...
def extract_emotion_and_text(response: str):
    ...
    # Whitelist of valid emotions to prevent "S" or other junk
    VALID_EMOTIONS = ['neutral', 'happy', 'sad', 'angry', 'anxious', 'surprised', 'tired', 'calm', 'relaxed', 'frustrated']
    
    emotion = "neutral"
    # Try bracketed format first
    emo_match = re.search(r'\[([\w\s]+)\]', response)
    if emo_match:
        found = emo_match.group(1).strip().lower()
        # If it's a long string like "Emotion Category: Happy", extract the last word
        if ' ' in found: found = found.split()[-1]
        if found in VALID_EMOTIONS:
            emotion = found
        else:
            # Try to see if any valid emotion is IN the found string
    ...
    if emotion == "neutral":
        ...
        emotion_keywords = {
            'happy': ['happy', 'joy', 'cheerful', 'smile', 'grin', 'laugh', 'excited'],
            'sad': ['sad', 'unhappy', 'down', 'depressed', 'upset', 'crying'],
            ...
            'surprised': ['surprised', 'shocked', 'astonished', 'wow'],
    ...
    # Aggressive truncation for over-explaining models
    truncation_markers = [
        "For the video you provided",
        "Based on the",
    ...
    leak_patterns = [
        r'^(Sure!|Certainly!|Of course!|I\'d be happy to help!|Understood!|Here\'s my response:?|Here is my response:?|I can continue the conversation.*?\.)',
        r'(?i)^Based on (the image|the video|your|what).*?(\.\s*|\:\s*)',
        r'(?i)^I (can|see|detect|notice|observe).*?(\.\s*|\:\s*)',
        r'(?i)^Please share the exact words.*?',
        r'(?i)^Sure, I can analyze.*?',
    ...
    text = re.sub(r'[*_#]', '', text).strip() # Remove markdown that breaks TTS
...
def _build_video_from_image_audio(image_path: str, audio_path: str, out_path: str) -> bool:
    ...
    Returns True on success. Uses ffmpeg directly for speed if available.
    """
    try:
        # Try direct ffmpeg first (much faster than moviepy)
        import subprocess
        cmd = [
            "ffmpeg", "-y", "-loop", "1", "-i", image_path,
            "-i", audio_path, "-c:v", "libx264", "-tune", "stillimage",
            "-c:a", "aac", "-b:a", "128k", "-pix_fmt", "yuv420p", "-shortest", out_path
        ]
        ...
        logger.info(f"  FFmpeg direct failed (expected if not in PATH): {e}. Falling back to MoviePy...")
        try:
            ...
            logger.warning(f"  Video build failed: {e2}")
            return False
...
async def _process_local(msg_type: str, msg_data: str, history: list):
    ...
    try:
        if msg_type == 'image':
            ...
            logger.info(f"💬 Chat text: '{msg_data[:60]}'...")
            start = time.time()

            history.append({"role": "user", "content": msg_data})

            # Build history-aware prompt (last 3 exchanges)
            history_context = ""
            if len(history) > 2:
                recent = history[-6:-1]
                lines = []
                for h in recent:
                    speaker = "User" if h['role'] == 'user' else "EmoAI"
            ...
            video_path = "None"  # text-only: no video needed
            ...
            logger.info(f"✅ Chat: {elapsed:.1f}s → [{emotion}] '{response_text[:50]}'")
